...
if(isset($_POST['bsimpan']))
    {
       ...
       {
           //data akan diedit
           $edit= mysqli_query($koneksi, "UPDATE ganti_pass set
                                         passwordbaru = '$_POST[tpasswordbaru]' ,
                                         confirmpassword = '$_POST[tconfirmpassword]' ,
...
document.location='ganti_pass.php';
       ...
       else
       {
           ...
           $simpan= mysqli_query($koneksi, "INSERT INTO ganti_pass (password baru,confirm password)
                                 VALUES ('$_POST[tpasswordbaru]', 
                                        '$_POST[tconfirmpassword]') 
    ...
    if(isset($_GET['hal']))
  {
    ...
    //Tampilkan data yang akan diedit
      $tampil = mysqli_query($koneksi, "SELECT * FROM ganti_pass WHERE id_mhs = '$_GET[id]' ");
      $data = mysqli_fetch_array($tampil);
      if($data)
      {
          //jika data ditemukan,maka data ditampung ke dalam variabel
          $vpasswordbaru = $data['password baru'];
          $vconfirmpassword = $data['confirm password'];
          $valamat = $data['alamat'];
          $vprodi = $data['prodi'];
    ...
    $hapus = mysqli_query($koneksi, "DELETE FROM ganti_pass WHERE id_mhs='$_GET[id]' ");
    if($hapus){
        echo "<script>
